...
export default function ERRequestManagementPage() {
  const [statusFilter, setStatusFilter] = useState("all")
  const [priorityFilter, setPriorityFilter] = useState("all")
  const [capabilityFilter, setCapabilityFilter] = useState("all")
  const [typeFilter, setTypeFilter] = useState("er")
  ...
  const fetchRequests = async () => {
    setLoading(true)
    try {
      console.log('Fetching ER requests with filters:', {
        status: statusFilter,
  ...
  const fetchStatusCounts = async () => {
    try {
      const pendingResponse = await fetch(`/api/requests/manage?status=pending receive sample&type=er&limit=1`)
      const inProgressResponse = await fetch(`/api/requests/manage?status=in-progress&type=er&limit=1`)
      const completedResponse = await fetch(`/api/requests/manage?status=completed&type=er&limit=1`)
      const rejectedResponse = await fetch(`/api/requests/manage?status=rejected&type=er&limit=1`)
      const terminatedResponse = await fetch(`/api/requests/manage?status=terminated&type=er&limit=1`)
  ...
  useEffect(() => {
    fetchRequests()
    fetchStatusCounts()
  }, [statusFilter, priorityFilter, capabilityFilter, typeFilter, currentPage, pageSize])
      ...
      <div className="flex flex-col">
        ...
        <div className="container px-4 py-6 md:py-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
              <h1 className="text-3xl font-bold tracking-tight">ER Request Management</h1>
              <p className="text-muted-foreground">Manage and track equipment reservation requests in the PCRD system</p>
            </div>
              ...
              <Card className="shadow-sm">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <CardTitle>
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="bg-green-50 text-green-800 hover:bg-green-50 border-green-200">ER</Badge>
                        {statusFilter === "all"
                          ? "All ER Requests"
                          : statusFilter === "pending receive sample"
                            ? "Pending ER Requests"
                            : statusFilter === "in-progress"
                              ? "In Progress ER Requests"
                              : statusFilter === "completed"
                                ? "Completed ER Requests"
                                : statusFilter === "rejected"
                                  ? "Rejected ER Requests"
                                  : statusFilter === "approved"
                                    ? "Approved ER Requests"
                                    : "ER Requests"}
                      </div>
                    </CardTitle>
                    <div className="flex items-center gap-2">
                      <div className="relative w-64">
                        ...
                        <Input
                          placeholder="Search ER requests..."
                          className="pl-10"
                          value={searchQuery}
